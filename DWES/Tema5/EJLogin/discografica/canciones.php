...
if ($_SESSION) {
    ...
    if ($_POST) {
        ...
        if(isset($_COOKIE)){
            setcookie('miCockie['.(sizeof($_COOKIE['miCockie'])+1).']', serialize($arrayCockies),time() +3600);
        }
    } else {
        getForm();
        if(isset($_COOKIE['miCockie'])){
            foreach($_COOKIE['miCockie'] as $nombre => $valor) {
                $titulo = htmlspecialchars($nombre);
                $genero = htmlspecialchars($valor);
                echo'Array'. $nombre .''. $genero .'<br>';
        }
    }
    }
} else {
